...
impl Decycler {
    ...
    pub fn compute_blocks(&mut self) {
        let vec_ci: Vec<Complex> = init_vec_ci(self.m);
...
fn compute_block(i: usize, block: &mut Vec<u64>, m: u16, vec_ci: &Vec<Complex>) {
...
///computes the center of mass of a kmer at the beginning of a sequence, before passing it to the
///next ones for "rolling" computations
pub fn compute_first_decycler(kmer: u64, m: u16, vec_ci: &Vec<Complex>) -> (bool, Complex) {
    let mut center_of_mass: Complex = Complex::new(0.0, 0.0); 
    for i in 0..m {
        let x_i: u64 = (kmer>>(2*i))&3;
        center_of_mass.add(&vec_ci[i as usize].mult_u64(x_i));
    }

    let is_decycler: bool = center_of_mass.check_decycler(&vec_ci[vec_ci.len()-1]);
    (is_decycler, center_of_mass)
}

///takes in the new kmer and current center of mass, and updates the new center of mass as well as
///returning if the new kmer belongs to the MDS
pub fn roll_decycler(x_remove: u64, x_new: u64, center_of_mass: &mut Complex, vec_ci: &Vec<Complex>)
    -> bool {
    center_of_mass.sub_u64(x_remove);
    center_of_mass.mult(&vec_ci[1]);
    center_of_mass.add(&vec_ci[vec_ci.len()-1].mult_u64(x_new));
...
///computes the center of mass of x, sees if the imaginary part is positive, rotates it and checks
///if the rotated imaginary part is negative
pub fn compute_membership(kmer: u64, m: u16, vec_ci: &Vec<Complex>) -> bool {
    let epsilon: f64 = 0.00001;
    let mut center_of_mass: Complex = Complex::new(0.0, 0.0);
    for i in 0..m {
        let x_i: u64 = (kmer>>(2*i))&3;
...
///now switching to complexes, and therefore this vec will be all the "rotation complexes" i need
///to compute my center of masses
pub fn init_vec_ci(m: u16) -> Vec<Complex> {
    let mut vec_ci: Vec<Complex> = Vec::with_capacity(m as usize);
    for i in 0..m {
        let teta: f64 = 2.0*std::f64::consts::PI*(i/m) as f64;
        vec_ci.push(Complex{a: teta.cos(), b: teta.sin()});
    }
    vec_ci
}
