...
METRIC_PATTERNS = {
    ...
    "false_positives": r"false positive rate : ([0-9eE+.\-]+)",
}
...
def plot_rows(rows: list[dict[str, object]], output_path: Path) -> None:
    ...
    query_cpu = [float(row["false_positives"]) for row in rows]

    fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharex=True)

    axes[0].plot(x_values, query_cpu, marker="o", label="superbloom")
    axes[0].set_title("Index Phase")
    axes[0].set_xlabel("size exponent")
    axes[0].set_ylabel("fp rate")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()

    fig.suptitle("bloomybloom benchmark: fp filter size sweep")
    fig.tight_layout()
